...
def parse_extract(idioms, sentences):
	...
	for sentences in documents:
		...
		print ('\nDone! Parsing document took : {0:.2f} seconds').format(time.time() - time_0)
...
if __name__=='__main__':
	...
	print('{0} idioms in : {1:.2f} seconds'.format(len(extracted_idioms), time.time() - extraction_start))
	idiom_set = set([extracted_idiom['idiom'] for extracted_idiom in extracted_idioms])
	if len(idiom_set) >= 5:
		idiom_sample = random.sample(idiom_set, 5)
		print('IDIOMS :: {0}, {1}, {2}, {3}, {4}'.format(u8(idiom_sample[0]), u8(idiom_sample[1]), u8(idiom_sample[2]), u8(idiom_sample[3]), u8(idiom_sample[4])))
